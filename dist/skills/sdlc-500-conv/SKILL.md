---
name: sdlc-500-conv
description: 将实际实现与需求、方案及任务进行比较，识别剩余差距，并将修正任务追加到任务清单。
...
使用本次已加载 SKILL.md 的绝对路径；Claude 可使用宿主替换后的 `${CLAUDE_PLUGIN_ROOT}`。插件包根目录是该 Skill 所在目录向上两级（`skills/sdlc-500-conv/`）。把该绝对目录绑定为 `SDLC_PLUGIN_ROOT`，不改变业务工作目录，不搜索另一安装版本。

在执行任何流程动作前，调用以下只读加载器，读取其完整标准输出（COMPLETE stdout）：

```sh
python3 -I -B "${SDLC_PLUGIN_ROOT:?}/scripts/python/load_workflow.py" --host "${SDLC_HOST:?}" --skill sdlc-500-conv
```
