...
if $REQUIRE_SPEC && [[ ! -f "$FEATURE_SPEC" ]]; then
    ...
    echo "Run $(format_sdlc_command sdlc-100-spec "$REPO_ROOT") first to create the feature specification." >&2
    exit 1
fi
...
if $REQUIRE_TASKS && [[ ! -f "$TASKS" ]]; then
    ...
    echo "Run $(format_sdlc_command sdlc-300-task "$REPO_ROOT") first to create the task list." >&2
    exit 1
fi
