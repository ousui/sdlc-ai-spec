...
with tempfile.TemporaryDirectory(prefix='rls-independent-probes-') as d:
    ...
    program='import sys; from pathlib import Path; sys.path.insert(0,sys.argv[1]); from tools.rls_validation_support import write_json; write_json(Path(sys.argv[2]), {"diagnostic":sys.argv[-1],"nested":{"password":sys.argv[-1]}}); sys.stdout.write(Path(sys.argv[2]).read_text())'
