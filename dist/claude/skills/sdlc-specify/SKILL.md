...
This is the claude package of SDLC's fixed Spec Kit core port. Resolve paths before executing the unchanged workflow below.

- Use the host-provided `${CLAUDE_PLUGIN_ROOT}` or the absolute path of this loaded SKILL.md. `SDLC_PLUGIN_ROOT` is the package directory two levels above this skill's directory (`skills/<name>/SKILL.md`). Do not infer it from the business working directory, scan other installed versions, or assume this variable is already exported.
- Keep the shell working directory in the selected business project. Run `SDLC_HOST=claude bash "${SDLC_PLUGIN_ROOT:?}/scripts/bash/project-paths.sh"` to resolve the nearest initialized `.sdlc`. When the user explicitly selected a project, pass its absolute path as command-local `SPECIFY_INIT_DIR`. Use the returned `PROJECT_ROOT` as `SDLC_PROJECT_ROOT`. If the project is missing or invalid, stop; this engineering build does not yet include `sdlc-init`.
...
- When constructing command invocations from hook command names, replace dots (`.`) with hyphens (`-`). For example, `sdlc.git.commit` → `/sdlc:sdlc-git-commit`.
...
The text the user typed after `/sdlc:sdlc-specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
3. **Create the spec feature directory**:
   ...
   Specs live under the default `.sdlc/specs/` directory unless the user explicitly provides `SPECIFY_FEATURE_DIRECTORY`.
   ...
   **Resolution order for `SPECIFY_FEATURE_DIRECTORY`**:
   ...
   2. Otherwise, auto-generate it under `.sdlc/specs/`:
      ...
      - Set `SPECIFY_FEATURE_DIRECTORY` to `.sdlc/specs/<directory-name>`
      - If `branch_numbering` was used (and `feature_numbering` was absent), emit a one-line warning: "⚠️ `branch_numbering` in init-options.json is deprecated. Rename to `feature_numbering`."

   **Create the directory and spec file**:
   - `mkdir -p SPECIFY_FEATURE_DIRECTORY`
   - Resolve the active `spec-template` through the Spec Kit preset/template resolution stack (equivalent to `SDLC_HOST=claude SPECIFY_INIT_DIR="${SDLC_PROJECT_ROOT:?}" bash "${SDLC_PLUGIN_ROOT:?}/scripts/bash/resolve-template.sh" spec-template`)
   ...
   - Persist the resolved path to `.sdlc/feature.json`:
     ...
     ```
     Write the actual resolved directory path value (for example, `.sdlc/specs/003-user-auth`), not the literal string `SPECIFY_FEATURE_DIRECTORY`.
     This allows downstream commands (`/sdlc:sdlc-plan`, `/sdlc:sdlc-tasks`, etc.) to locate the feature directory without relying on git branch name conventions.

   **IMPORTANT**:
   - You must only create one feature per `/sdlc:sdlc-specify` invocation
...
8. **Specification Quality Validation**: After writing the initial spec, validate it against quality criteria:
   ...
   a. **Create Spec Quality Checklist**: Generate a checklist file at `SPECIFY_FEATURE_DIRECTORY/checklists/requirements.md` using the checklist template structure with these validation items:
      ...
      **Feature**: [Link to spec.md]
      ...
      - Items marked incomplete require spec updates before `/sdlc:sdlc-clarify` or `/sdlc:sdlc-plan`
      ```
...
- Readiness for the next phase (`/sdlc:sdlc-clarify` or `/sdlc:sdlc-plan`)
