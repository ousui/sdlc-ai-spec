---
name: sdlc-110-clar
description: 针对当前功能规格中说明不足的部分，提出最多 5 个高度聚焦的澄清问题，并将答案写回规格。
...
argument-hint: 可选：需要澄清的规格部分
---

# SDLC AI SPEC sdlc-110-clar
...
使用本次已加载 SKILL.md 的绝对路径；Claude 可使用宿主替换后的 `${CLAUDE_PLUGIN_ROOT}`。插件包根目录是该 Skill 所在目录向上两级（`skills/sdlc-110-clar/`）。把该绝对目录绑定为 `SDLC_PLUGIN_ROOT`，不改变业务工作目录，不搜索另一安装版本。

在执行任何流程动作前，调用以下只读加载器，读取其完整标准输出（COMPLETE stdout）：

```sh
python3 -I -B "${SDLC_PLUGIN_ROOT:?}/scripts/python/load_workflow.py" --host "${SDLC_HOST:?}" --skill sdlc-110-clar
```
