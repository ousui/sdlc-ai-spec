...
@contextmanager
def resource_budget_only():
    """Disable sandbox setup locally; never overwrite process-wide sys.platform.

    The capture helper must retain the real OS identity so Darwin's existing
    process-group cleanup fallback remains available in these budget tests.
    """
...
class NativeResourceBudgetTests(unittest.TestCase):
    def test_budget_mock_does_not_change_process_capture_platform(self):
        import vfy_executor
        import vfy_process_capture
        host_platform = sys.platform
        with resource_budget_only():
            self.assertEqual('resource-test', vfy_executor.sys.platform)
            self.assertEqual(host_platform, vfy_process_capture.sys.platform)
            self.assertEqual(host_platform, sys.platform)
