...
加载窗口并检查 Customize。不要只复制 skills，也不要把旧插件文件合并进同一目录。
...
Claude 可能在显式版本字符串不变时跳过更新。不要只凭界面版本判断 Update 已获取目标提交；应采用全新安装或文档规定的卸载/重装/重新加载流程，并核验 `version`、准确源码提交和 build_id。
...
目标来源安装新 ID，再核验 11 个入口和新 build_id。不要合并两个包目录树，也不要假定版本字符串本身会使缓存失效。不执行自动缓存移除或业务数据迁移，不宣传或安装
...
当前需求质量清单入口为 `sdlc-210-huma`；历史编号为 `sdlc-320-huma`，不保留运行时
别名。推荐在 PLAN 后、TASK 前使用，不是 CONV 后的功能验收；职责、清单路径与
