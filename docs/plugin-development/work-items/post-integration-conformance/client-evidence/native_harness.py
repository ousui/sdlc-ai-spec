...
def execute(skill, action, attempt):
    ...
    assert source_before['sha'] == SHA and source_before['status'] == '', source_before
    base = LAB / skill
    project = base / 'project'
    out = EVIDENCE / 'native' / skill
    ...
    if action not in ('marketplace', 'install'):
        observation['installed_snapshot_after'] = runtime_snapshot(cache, skill, 'codex-cli')
        observation['installed_unchanged'] = observation['installed_snapshot_after'] == fixture['runtime_snapshot_sha256']
    ...
    if action in ('marketplace', 'install'):
        print(receipt['stdout'][-1600:])
    elif action == 'discovery':
        print('Native prompt context archived:', len(receipt['stdout'].encode()), 'bytes')
    else:
        for line in receipt['stdout'].splitlines():
            ...
            if event.get('type') == 'item.completed' and item.get('type') == 'agent_message':
                print('Host message:', item.get('text', '')[-2000:])
    if receipt['exit_code'] != 0: print(receipt['stderr'][-1200:])
    return observation
