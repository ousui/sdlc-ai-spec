...
from build import ROOT,COMMANDS,HOSTS,UPSTREAM_SHA,REPOSITORY,VERSION,render_source,split,build,binding
...
def verify(upstream:Path,baselines:Path,evidence:Path)->dict:
    ...
    checks=[]
    def passed(group,item):checks.append({'group':group,'item':item,'result':'PASS'})
    ...
    for host,integ,folder in [('codex','codex','.agents'),('claude','claude','.claude'),('cursor','cursor-agent','.cursor')]:
        ...
        allowed={'name','version','description','repository','license'}
        if host!='claude':
            allowed.add('$schema')
