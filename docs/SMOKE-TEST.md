...
| 需求质量评审清单（按需；PLAN 后、TASK 前） | `$sdlc-210-huma` | `/sdlc-210-huma` |
...
勾选自己的审查门禁。HUMA 生成条目后停止，新条目保持未勾选；评审者逐项确认
需求质量，而不是等待代码实现后才勾选。可在评审者明确要求后让 Agent 协助评估。

分别验证清单有未勾选项和全部勾选的情况：IMPL 对前者报告并询问是否继续，等待
用户回答；对后者按原流程继续。IMPL 不修改清单标记。HUMA 不是强制前置阶段；
清单生成与人工评审是两个动作。CONV 后的运行验证/业务验收不是 HUMA。
参见 [使用流程与评审职责](USAGE.md)。
