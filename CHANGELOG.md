...
- 锁定上游 GitHub Spec Kit v1.0.7，产品版本按规则重置为 `1.0.7-sdlc.1`。
- 九个核心 command：若 `.sdlc/extensions.yml` 无法解析或无效，不得静默跳过钩子检查，须向用户报告解析错误（含强制钩子未检查）后继续。
- CLAR：将澄清推迟到规划阶段的条件收窄为实现方法、技术栈比较或任务拆解。
- RULE：同步影响报告明确为提交前应删除的临时审查草稿，不是宪法治理内容。
- 同步九个 command 的简体中文工作流译文；机器契约与输入别名未改。
- 上游版本重置时，`prepare` 将已接受源码的 Unreleased 笔记提升为候选变更记录，使产品版本与 `CHANGELOG.md` 进入同一冻结摘要。
