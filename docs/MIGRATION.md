...
执行。五类已审查模板呈现和内置需求清单已本地化。原始英文模板映射保留在 `src/templates`；分发模板使用 canonical 中文标题/字段，读取兼容旧英文与历史双语标题。占位符、代码路径、任务语法及其他机器契约独立检查。
...
上游 checklist 映射为 `sdlc-210-huma`，推荐在 PLAN 后、TASK 前生成需求质量评审
清单。编号迁移不是新增生命周期阶段或流程门禁；锁定的原始命令、前置检查、清单
追加规则、未勾选初始状态及评审者责任保持不变。IMPL 仍只读清单并在需要时询问
是否继续；CONV 仍只追加实现差距任务。编号历史见 [NAMING.md](NAMING.md)，使用与
完成边界见 [USAGE.md](USAGE.md)。
