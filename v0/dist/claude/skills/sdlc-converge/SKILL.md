...
`/sdlc:sdlc-implement` can complete it. This command MUST run only after
`/sdlc:sdlc-implement` has run on the current `tasks.md`, and after `/sdlc:sdlc-tasks` has produced a complete `tasks.md`.
...
prerequisite command to run (`/sdlc:sdlc-specify` for a missing spec, `/sdlc:sdlc-plan` for a missing plan,
`/sdlc:sdlc-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/sdlc:sdlc-implement` to complete them; note that a follow-up converge
