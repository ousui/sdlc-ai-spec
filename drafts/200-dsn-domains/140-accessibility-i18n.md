...
- Accessibility 和 Internationalization 按本 Spec 分别判断 Disposition；
- 子领域先分别判定，再按固定聚合规则形成 Domain Disposition；
- 新增或改变用户界面及交互时，Accessibility 通常为 `required`；
- 已有设计系统完整覆盖且当前变化没有新增或改变相关义务时，可以引用准确 Baseline 判定相应子领域为 `n/a`；
...
两个子领域的处置始终记录在父 DSN 主文件的固定 Composite Domain Subdomain Applicability 表；顶层 Domain 为 `required` 时才创建本 Domain 子文件承载详细设计。
...
- Accessibility 与 Internationalization 的 Disposition、Basis、原因和 Exception 只记录在父 DSN 主文件，不在本文件复制；
...
|---|---|---|---|
| DSN-DG-140-001 | 两个子领域的 Disposition、依据、N/A 原因和 Exception 完整一致 | pending |  |
| DSN-DG-140-002 | 适用且未豁免的可访问性、语言、Locale、区域格式及内容布局适配设计完整 | pending |  |
| DSN-DG-140-003 | 适用的回退与降级行为明确，并由 VFY Points 覆盖 | pending |  |
