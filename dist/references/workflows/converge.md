...
- When constructing command invocations from hook command names, replace dots (`.`) with hyphens (`-`). For example, `sdlc.git.commit` → `@@SDLC_BIND_0035@@sdlc-git-commit`.
...
`@@SDLC_BIND_0073@@sdlc-implement` can complete it. This command MUST run only after
`@@SDLC_BIND_0074@@sdlc-tasks` has produced a complete `tasks.md`.
...
- modify, create, or delete any application code — completing the appended tasks is the
  job of `@@SDLC_BIND_0088@@sdlc-implement`.
...
Run `SDLC_HOST=c@@SDLC_BIND_0102@@ SPECIFY_INIT_DIR="${SDLC_PROJECT_ROOT:?}" bash "${SDLC_PLUGIN_ROOT:?}/scripts/bash/check-prerequisites.sh" --json --require-spec --require-tasks --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS. Derive absolute paths:
...
prerequisite command to run (`@@SDLC_BIND_0109@@sdlc-plan` for a missing plan,
`@@SDLC_BIND_0110@@sdlc-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `@@SDLC_BIND_0239@@sdlc-implement` to complete them; note that a follow-up converge
...
- When constructing command invocations from hook command names, replace dots (`.`) with hyphens (`-`). For example, `sdlc.git.commit` → `@@SDLC_BIND_0256@@sdlc-git-commit`.
- For each executable hook, output the following based on its `optional` flag:
  - **Optional hook** (`optional: true`):
