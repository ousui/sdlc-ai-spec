...
上游 package version 发生变化时，`tools/upgrade.py prepare` 在 detached 候选中把产品版本重置为 `<新上游版本>-sdlc.1`，并把已接受源码 `CHANGELOG.md` 的 Unreleased 笔记提升为该版本的首条已发布记录，使机器版本、变更记录和构建身份进入同一 `candidate_digest`。准备前源码必须已写好 Unreleased 条目，并设置 `SDLC_RELEASE_TIME`（`YYYY-MM-DD HH:MM:SS +08:00`）；prepare 不编造发布说明。同一上游版本内的本地功能/本地化迭代不由升级工具自动猜测，由维护者使用 `tools/version-tool.sh sdlc.N` 显式设置本地迭代号。版本字符串不能替代准确提交 SHA 和 `BUILD.json.build_id`。
