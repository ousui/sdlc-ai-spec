...
def _analyze(candidate: Mapping[str, Any], phase_inputs: PhaseInputs):
    ...
    # Aggregation is derived from complete exact inputs, not candidate authority.
    authoritative_aggregate = tuple(phase_inputs.metadata.get("aggregated_applicability") or ())
    if any(row["disposition"] == "pending" for row in authoritative_aggregate):
        checks["PLN-G-005"] = _outcome("pending", "Upstream applicability is unresolved")
    for row in lifecycle:
        bound = next((item for item in authoritative_aggregate if item["phase"] == row.get("phase")), None)
        if bound and row.get("disposition") != bound["disposition"]:
            checks["PLN-G-005"] = _outcome("fail", "Lifecycle Applicability disagrees with authoritative aggregation")
    for row in aggregated:
        bound = next((item for item in authoritative_aggregate if item["phase"] == row.get("phase")), None)
        if bound is None or row.get("disposition") != bound["disposition"]:
            checks["PLN-G-005"] = _outcome("fail", "Candidate cannot replace authoritative aggregation")
    for row in delivery_scope:
        supplied = refs(row.get("source_references"), "delivery source")
        if row.get("source_artifact_reference"):
            supplied = (*supplied, row["source_artifact_reference"])
        if any(ref.split("#", 1)[0].split("/", 1)[0] not in phase_inputs.scope_references for ref in supplied):
            checks["PLN-G-001"] = _outcome("fail", "Delivery Source is outside the complete input set")
