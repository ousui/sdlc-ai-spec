...
class RepositoryTests(unittest.TestCase):
    ...
    def test_metadata_and_attribution(self):
        ...
        match=re.fullmatch(re.escape(str(lock['version']))+r'-sdlc\.([1-9][0-9]*)',m['version'])
        ...
        self.assertEqual(upstream['version'],lock['version'])
        self.assertEqual(upstream['port_revision'],revision)
        build=json.loads((ROOT/'dist/BUILD.json').read_text())
        self.assertEqual(build['product_version'],m['version'])
        self.assertEqual(build['upstream_version'],upstream['version'])
        self.assertEqual(build['port_revision'],revision)
