...
当前项目通过固定提交的 `actions/checkout` 拉取触发事件的准确 SHA，自动使用当前
仓库的 `GITHUB_TOKEN`，同时支持公开与私有仓库；检出后不保留认证凭据。
push 和 PR 在两个系统各执行一次完整 verifier（包含仓库单元测试与独立脚本比较）。
手动触发还会在两个系统执行同版本隔离升级演练，其中会再次验证候选；普通提交不重复演练。
