...
文件与 Artifact 目录使用 `000-ctx` 表示固定前置位置并保持自然排序；`000` 不是 Phase Order，`CTX` 也不进入 Phase Code 枚举。
...
- CTX Artifact 同时受 Core Spec 与 Project Context Spec 约束；
- CTX 的 Evaluation Contract Set 至少包含这两个 Spec 的准确不可变引用；
...
- CTX 复用 Core 的 Revision Index、Artifact Status、Evidence、Supporting Artifact Manifest、Exceptions、Open Items、Final Confirmation 和 Gate Summary，不另建平行控制结构。
...
`000-ctx` 只表示 Lifecycle 前置位置，不是 Phase。

Revision Index 复用 Core 固定结构：
...
1. CTX ID、目录、Front Matter 和 Revision 一致；
2. Revision Index 中存在唯一对应行且 `State=frozen`；
3. Status 为 `ready` 或 `ready_with_exception`；
4. Supporting Artifact Manifest、Evidence、Final Confirmation 和 Gate 可以验证；
