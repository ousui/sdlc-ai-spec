...
existing project `templates/overrides/constitution-template.md`. When INIT actually creates
that file, it also attempts to create `memory/.constitution-template.json` containing the
SHA-256 of the exact generated bytes and a source label. This is generation provenance only:
a matching hash does not prove that RULE never ran or that the file is unapproved, and a
different hash does not prove RULE completion. Existing constitutions are preserved verbatim;
missing provenance on an older/manual project is not backfilled from the current template.

The provenance sidecar is optional observation data, not a new lifecycle gate. If only the
sidecar write fails, the created constitution remains and INIT reports the missing provenance.
A malformed existing sidecar is preserved for review rather than silently rewritten. STATUS
may read it; RULE/SPEC/PLAN/TASK do not require it.
...
created/updated/preserved paths and warnings. `constitution_provenance` reports
`recorded`, `would_record`, `preserved`, `not_recorded`, or `unavailable` without
turning the optional sidecar into a core workflow gate. On failure it exits nonzero
with an error on stderr; a failed or dry-run call is not completed initialization.
