...
构建输入、升级源码摘要、翻译目录摘要及非翻译冻结摘要共用
`tools/source_files.py` 的窄范围规则：仅忽略名为 `.DS_Store` 的普通文件。
打包复制脚本、模板和资源时也排除这类文件；忽略不会删除原工作目录中的文件。
同名目录、符号链接、相似文件名、实际代码、配置和可执行权限仍按原规则检查，
不得扩大为忽略全部隐藏文件，也不根据 `.gitignore` 自动放宽候选摘要。

本地化子树中的真实译文或 catalog 变化仍会改变翻译摘要；非翻译源码变化仍会
阻止 `refresh-localization`。摘要算法或工具变更后须重新准备候选、重新验证，
不得重写旧候选记录或复用旧 PASS 证据。
...
上游 package version 发生变化时，`tools/upgrade.py prepare` 在 detached 候选中把产品版本重置为 `<新上游版本>-sdlc.1`，并把已接受源码 `CHANGELOG.md` 的 Unreleased 笔记提升为该版本的首条已发布记录，使机器版本、变更记录和构建身份进入同一 `candidate_digest`。准备前源码工作树必须干净且已写好 Unreleased 条目，并在环境中设置 `SDLC_RELEASE_TIME`（`YYYY-MM-DD HH:MM:SS +08:00`）；`prepare` 只读取该变量名，不编造发布说明，也不经其他别名间接赋值。缺少说明时需先独立授权补写并提交，不能假定一次调用即可完成跨版本候选。同版本演练不提升 CHANGELOG，也不要求该时间变量。同一上游版本内的本地功能/本地化迭代不由升级工具自动猜测，由维护者使用 `tools/version-tool.sh sdlc.N` 显式设置本地迭代号。版本字符串不能替代准确提交 SHA 和 `BUILD.json.build_id`。
...
翻译子树。补译后先运行候选自身的 `tools/localize.py precheck`；该步骤只读，只验证
当前机器/结构与本地关键职责边界，不更新旧摘要，也不代表语义审批。随后独立完成
源文/译文语义复核，再执行 `record` 和全量 `check`。

显式完成翻译审查后，`tools/upgrade.py refresh-localization --record ... --review ...`
验证已冻结的非翻译输入，重新检查中文并构建候选，生成新候选摘要，而不是新的已接受
版本。旧测试证据不得复用。完整命令和审查记录格式见 [LOCALIZATION.md](LOCALIZATION.md)。
审查字符串是流程记录，不是身份认证。任何机器契约、关键职责或新结构检查失败都必须
先审查原因；不得在升级候选中修改 checker、测试或忽略规则来把失败改成通过。
...
模板、固定响应及需求示例的新鲜度参与 LOCALIZATION_REQUIRED 检查。Localization Contract v2 把自然语言呈现与机器契约分开：英文标题/标签可以映射为 canonical 中文，但命令、路径、ID、配置 token、状态枚举和结构层级仍必须通过机器契约检查。`catalog.json` 同时保存 canonical 结构映射、历史英文/双语读取别名和已审查输入别名；升级不得把未知标题或新机器 token 静默当作普通文案。

`localize.py export` 包含这些英文输入；`record --presentation NAME` 在既有本地化专用刷新前记录已完成的来源审查。未变化译文可以复用；来源发生变化时只允许对候选本地化子树做受控补译。新本地化模板位于已授权的本地化子树；刷新不允许修改其他位置或手工编辑候选摘要。
