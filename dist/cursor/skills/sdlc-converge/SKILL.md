...
`/sdlc-implement` can complete it. This command MUST run only after
`/sdlc-implement` has run on the current `tasks.md`, and after `/sdlc-tasks` has produced a complete `tasks.md`.
...
**Constitution Authority**: The project constitution (`.sdlc/memory/constitution.md`) is
...
- CONSTITUTION = `.sdlc/memory/constitution.md` (if present)
If `spec.md`, `plan.md`, or `tasks.md` is missing, STOP with a clear, actionable message naming the
prerequisite command to run (`/sdlc-specify` for a missing spec, `/sdlc-plan` for a missing plan,
`/sdlc-tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/sdlc-implement` to complete them; note that a follow-up converge
...
After producing the result, check if `.sdlc/extensions.yml` exists in the project root.
