...
1. **准备**：在仓库根目录运行 `SDLC_HOST={{SDLC:HOST}} SDLC_INIT_DIR="${SDLC_PROJECT_ROOT:?}" bash "${SDLC_PLUGIN_ROOT:?}/scripts/bash/check-prerequisites.sh" --json --template checklist-template`，解析 JSON 中的 FEATURE_DIR、AVAILABLE_DOCS 列表及 TEMPLATE_CONTENT。
   - 全部文件路径必须为绝对路径。
...
7. **结构参考**：遵循 `${SDLC_PLUGIN_ROOT}/templates/checklist-template.md` 的规范模板，生成标题、元数据区、类别标题、责任说明、备注及 ID 格式。模板不可用时采用：H1 标题、用途／创建日期元数据行、解释 `[x]` 代表评审者认可需求质量的责任说明、包含 `- [ ] CHK### <requirement item>` 行的 `##` 类别章节；ID 从 CHK001 开始全局递增；备注说明 `{{SDLC:IMPLEMENT}}` 读取清单状态但不修改标记。
...
**重要**：每次 `{{SDLC:CHECKLIST}}` 调用使用简短、描述性文件名，新建或追加至已有清单。这样可以：
