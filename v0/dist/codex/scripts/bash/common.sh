...
# Safely read .sdlc/feature.json's "feature_directory" value.
...
# Persist a feature_directory value to .sdlc/feature.json.
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .sdlc/feature.json,
...
#   1. .sdlc/templates/overrides/
#   2. .sdlc/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .sdlc/extensions/<ext-id>/templates/
#   4. .sdlc/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.sdlc/templates"
    ...
    local presets_dir="$repo_root/.sdlc/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.sdlc/extensions"
    if [ -d "$ext_dir" ]; then
    ...
    local core="$(_sdlc_plugin_root)/templates/${template_name}.md"
