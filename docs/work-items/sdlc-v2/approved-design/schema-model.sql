-- SDLC v2 detailed-design schema model
-- Generated for design validation, not a production migration.
-- Runtime validators remain required; see DATABASE.md.
...
CREATE TABLE context_entries (
  ...
  kind TEXT NOT NULL CHECK(kind IN ('fact','rule','resource','command')),
  name TEXT NOT NULL,
...
CREATE TABLE changes (
  ...
  initial_base_commit TEXT,
  delivery_mode TEXT NOT NULL CHECK(delivery_mode IN ('local','git','deployment')),
...
CREATE TABLE sources (
  ...
  kind TEXT NOT NULL CHECK(kind IN ('text','document','image','observation','assumption')),
...
CREATE TABLE requirements (
  ...
  kind TEXT NOT NULL CHECK(kind IN ('behavior','rule','quality','constraint')),
...
CREATE TABLE tasks (
  ...
  target_phase TEXT NOT NULL CHECK(target_phase IN ('IMP','VFY','RLS')),
  kind TEXT NOT NULL CHECK(kind IN ('prepare','implement','verify','review','deliver')),
  title TEXT NOT NULL,
...
CREATE TABLE checks (
  ...
  method TEXT NOT NULL CHECK(method IN ('test','inspection','analysis','demonstration')),
  executor TEXT NOT NULL CHECK(executor IN ('command','agent','human')),
  description TEXT NOT NULL,
  expected_result TEXT NOT NULL,
  argv_json TEXT,
...
CREATE TABLE preconditions (
  ...
  enforce_at TEXT NOT NULL CHECK(enforce_at IN ('start','execute','complete')),
  reason TEXT NOT NULL,
  FOREIGN KEY (revision_id) REFERENCES revisions(revision_id),
...
CREATE TABLE runs (
  ...
  review_mode TEXT NOT NULL CHECK(review_mode IN ('auto','assisted')),
  error_code TEXT,
  error_message TEXT,
...
CREATE TABLE steps (
  ...
  change_id TEXT,
  run_id TEXT NOT NULL,
  phase TEXT NOT NULL CHECK(phase IN ('INIT','CTX','REQ','DSN','PLN','IMP','VFY','RLS')),
  ...
  outcome TEXT CHECK(outcome IN ('pass','fail','not_applicable','unknown')),
  started_at TEXT NOT NULL,
  finished_at TEXT,
...
CREATE TABLE check_results (
  ...
  snapshot_id TEXT,
  status TEXT NOT NULL CHECK(status IN ('pass','fail','blocked','unknown')),
  evidence_asset_id TEXT NOT NULL,
  source_kind TEXT NOT NULL CHECK(source_kind IN ('command','agent','human','reused')),
  reused_from_id TEXT,
...
CREATE TABLE findings (
  ...
  design_id TEXT,
  task_id TEXT,
  return_phase TEXT NOT NULL CHECK(return_phase IN ('REQ','DSN','PLN','IMP','VFY','RLS')),
  kind TEXT NOT NULL CHECK(kind IN ('missing','partial','contradicts','unrequested','environment')),
  severity TEXT NOT NULL CHECK(severity IN ('blocking','advisory')),
  status TEXT NOT NULL CHECK(status IN ('open','addressed','resolved','rejected')),
...
CREATE TABLE deliveries (
  ...
  vfy_result_id TEXT NOT NULL,
  mode TEXT NOT NULL CHECK(mode IN ('local','git','deployment')),
  target TEXT NOT NULL,
  effect_key TEXT NOT NULL,
  status TEXT NOT NULL CHECK(status IN ('prepared','succeeded','failed','unknown','cancelled')),
  readback_result_id TEXT,
  summary TEXT NOT NULL,
...
CREATE TABLE imports (
  ...
  status TEXT NOT NULL CHECK(status IN ('imported','conflict','rejected')),
  summary TEXT NOT NULL,
  created_at TEXT NOT NULL
) STRICT;
...
CREATE INDEX dependencies_reverse ON task_dependencies(revision_id,predecessor_id);

CREATE TRIGGER lock_sources_insert BEFORE INSERT ON sources
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_sources_update BEFORE UPDATE ON sources
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_sources_delete BEFORE DELETE ON sources
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_sources_move BEFORE UPDATE ON sources
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_requirements_insert BEFORE INSERT ON requirements
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirements_update BEFORE UPDATE ON requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirements_delete BEFORE DELETE ON requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirements_move BEFORE UPDATE ON requirements
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_criteria_insert BEFORE INSERT ON criteria
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criteria_update BEFORE UPDATE ON criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criteria_delete BEFORE DELETE ON criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criteria_move BEFORE UPDATE ON criteria
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_requirement_sources_insert BEFORE INSERT ON requirement_sources
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirement_sources_update BEFORE UPDATE ON requirement_sources
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirement_sources_delete BEFORE DELETE ON requirement_sources
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_requirement_sources_move BEFORE UPDATE ON requirement_sources
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_criterion_requirements_insert BEFORE INSERT ON criterion_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criterion_requirements_update BEFORE UPDATE ON criterion_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criterion_requirements_delete BEFORE DELETE ON criterion_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_criterion_requirements_move BEFORE UPDATE ON criterion_requirements
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_designs_insert BEFORE INSERT ON designs
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_designs_update BEFORE UPDATE ON designs
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_designs_delete BEFORE DELETE ON designs
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_designs_move BEFORE UPDATE ON designs
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_design_requirements_insert BEFORE INSERT ON design_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_design_requirements_update BEFORE UPDATE ON design_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_design_requirements_delete BEFORE DELETE ON design_requirements
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_design_requirements_move BEFORE UPDATE ON design_requirements
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_tasks_insert BEFORE INSERT ON tasks
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_tasks_update BEFORE UPDATE ON tasks
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_tasks_delete BEFORE DELETE ON tasks
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_tasks_move BEFORE UPDATE ON tasks
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_task_designs_insert BEFORE INSERT ON task_designs
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_designs_update BEFORE UPDATE ON task_designs
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_designs_delete BEFORE DELETE ON task_designs
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_designs_move BEFORE UPDATE ON task_designs
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_task_criteria_insert BEFORE INSERT ON task_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_criteria_update BEFORE UPDATE ON task_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_criteria_delete BEFORE DELETE ON task_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_criteria_move BEFORE UPDATE ON task_criteria
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_task_dependencies_insert BEFORE INSERT ON task_dependencies
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_dependencies_update BEFORE UPDATE ON task_dependencies
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_dependencies_delete BEFORE DELETE ON task_dependencies
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_task_dependencies_move BEFORE UPDATE ON task_dependencies
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_checks_insert BEFORE INSERT ON checks
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_checks_update BEFORE UPDATE ON checks
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_checks_delete BEFORE DELETE ON checks
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_checks_move BEFORE UPDATE ON checks
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_check_criteria_insert BEFORE INSERT ON check_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_check_criteria_update BEFORE UPDATE ON check_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_check_criteria_delete BEFORE DELETE ON check_criteria
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_check_criteria_move BEFORE UPDATE ON check_criteria
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;

CREATE TRIGGER lock_preconditions_insert BEFORE INSERT ON preconditions
WHEN (SELECT state FROM revisions WHERE revision_id=NEW.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_preconditions_update BEFORE UPDATE ON preconditions
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_preconditions_delete BEFORE DELETE ON preconditions
WHEN (SELECT state FROM revisions WHERE revision_id=OLD.revision_id)='committed'
BEGIN SELECT RAISE(ABORT,'committed revision is immutable'); END;

CREATE TRIGGER lock_preconditions_move BEFORE UPDATE ON preconditions
WHEN NEW.revision_id<>OLD.revision_id
BEGIN SELECT RAISE(ABORT,'revision identity cannot change'); END;
