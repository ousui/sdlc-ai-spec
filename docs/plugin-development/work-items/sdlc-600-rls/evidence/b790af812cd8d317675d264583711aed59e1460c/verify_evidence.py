...
def verify_propagation(repo, evidence, result, archived, reports):
    ...
    for prefix, stem in ((raw, ''), (raw / 'attest-steps', 'fresh-')):
        for filename, count in (('rls-private.json', 435), ('repository.json', 1068)):
            ...
            # unittest emits shortDescription on a second line for VFY cases.
            # Require an explicit terminal ok for both formats, with all IDs unique.
            passed = re.findall(r'^test_\w+ \(([^)]+)\)(?: \.\.\. ok|\n[^\n]+ \.\.\. ok)$', suite['log'], re.M)
